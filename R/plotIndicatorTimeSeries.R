...
#  plotIndicatorTimeSeries("amo.csv", coltoplot=2, plotrownum=1, plotcolnum=1,
#     adjlabmain=1, sublabel=F)
#  plotIndicatorTimeSeries("aveSocialConnectedness.csv", coltoplot=2,
#     plotrownum=1, plotcolnum=1, sublabel=F)
#  plotIndicatorTimeSeries("menhaden_abundance_index.csv", coltoplot=2,
#     plotrownum=1, plotcolnum=1, sublabel=F, yposadj=0.8, widadj=0.8)
#  plotIndicatorTimeSeries("seagrass_acreage.csv", coltoplot=2:7, plotrownum=3,
#     plotcolnum=2, adjlabmain=1, sublabel=T, widadj=0.6, trendAnalysis=F)
#                                                                              #
#                                                                              #
################################################################################

plotIndicatorTimeSeries <-  function(
  filename, coltoplot=2, sublabel=F, plotrownum = 1, plotcolnum = 1,
  yposadj=1, widadj=1, hgtadj=1, trendAnalysis=T, redgreen=T, outname=NA,
  sameYscale=F
){

    # load data file
    d1 <- read.table(filename, header=F, sep=",", skip=0, quote="")
    # load data file labels
    d <- read.table(filename, header=F, sep=",", skip=3, quote="")
    ...
    if ( class(tim_all)=="factor" )  {
        monthly <- T
    }   else  {
        monthly <- F
    }
    if ( class(tim_all)=="factor" )  {
        tim_all <- as.numeric(substr(tim_all,4,7)) + (match(substr(tim_all,1,3), month.abb)-1)/12
    }
    ...
    if (plotcolnum + plotrownum > 2)  {
        plotcolnum2 <- plotcolnum*0.65; plotrownum2 <- plotrownum*0.65
    }  else {
        plotcolnum2 <- plotcolnum; plotrownum2 <- plotrownum
    }
    # set graphics specifications
    if (is.na(outname))  {
        filnam <- paste(c(unlist(strsplit(filename, ".csv"))), ".pdf", sep="")
    }   else   {  filnam <- outname  }

    # layout for single or multi-panel plots
    nf <- layout(
        matrix(c(1:(plotrownum*plotcolnum*2)),
        plotrownum, plotcolnum*2, byrow = TRUE),
  ...
  # layout for single plots with fewer than 5 data points or no trend analysis
    if (length(coltoplot)==1 & length(tim_all) <= 5 | trendAnalysis==F)  {
        nf <- layout(
            matrix(c(1:(plotrownum*plotcolnum)), plotrownum, plotcolnum, byrow = TRUE),
...
for (i in coltoplot)  {
    ...
    if (sum(!is.na(co_all)) == 0) {  plot.new(); plot.new()  }  else {
        ...
        if (length(tim) > 5) {
            if (trendAnalysis==T)  {
                par(mar=c(2.5,5,3,0), xpd=F)
            }  else  {  par(mar=c(2.5,5,3,1), xpd=F)  }
            par(mgp=c(3*yposadj,1,0))
            if (sublabel==T) {
                mm <- paste(as.character(d1[1,i]), "\n", as.character(d1[3,i]))
            } else { mm <- d1[1,i] }
            if (sameYscale==T)  {   # plot time series
                plot(tim_all, co_all, col=0, axes=F, xlab="", ylab=d1[2,i], main=mm, ylim=c(ymin, ymax))
            }
            if (sameYscale==F)  {
                plot(tim_all, co_all, col=0, axes=F, xlab="", ylab=d1[2,i], main=mm)                        }                   # plot time series
                ...
                polygon(
                    c(
                        min(tim_all, na.rm=T)-5,
                        max(tim_all, na.rm=T)+5,
                        max(tim_all, na.rm=T)+5,
                        min(tim_all, na.rm=T)-5
                    ), c(
                ...
                if (trendAnalysis==T)  {
                    polygon(
                        ...
                        col="#0000FF20", border=F
                    )
                }
                if (mean(diff(tim_all)) <= 1.1 )  {  # plot time series
                    lines(tim_all, co_all, lwd=2);
                    points(tim_all, co_all, pch=20, cex=0.75)
                }
                if (mean(diff(tim_all)) > 1.1  )  {
                    points(tim_all, co_all, pch=20, cex=1.5)
                }
                abline(h=mean(co, na.rm=T), lty=8);
                abline(h=mean(co, na.rm=T)+sd(co, na.rm=T), lty=1);
                abline(h=mean(co)-sd(co), lty=1)
                if (length(tim) > 10)  {
                    axis(1, at=seq(1900, 2015, 5))
                } else {
                    axis(1, at=seq(1900, 2016, 2))
                }
                # add axes
                axis(1, at=seq(1900, 2016, 1), tck=-0.015, lab=rep("", 117))
                axis(2, las=2); box()

                if (trendAnalysis==T)  {
                    par(mar=c(2.5,0,3,0))    #  second panel on mean and trend of last 5 years
                    if (monthly == F)  {
                        last5 <- co_all[(nrow(d)-4):nrow(d)]
                    }  else  {
                        last5 <- co_all[(nrow(d)-59):nrow(d)]
                    }
                    ...
                    if (sum(is.na(last5)) < 2)  {
                        if (mean(last5, na.rm=T) > (mean(co, na.rm=T)+sd(co, na.rm=T)))  {
                            text(1, 1.2, col="white", "+", cex=2.6, font=2)
                        }
                        if (mean(last5, na.rm=T) < (mean(co, na.rm=T)-sd(co, na.rm=T)))  {
                            text(1, 1.2, col="white", "-", cex=2.6, font=2)
                        }
                        if (monthly == F)  {
                            res   <- summary(lm(last5~tim[1:5]))
                        } else {    res   <- summary(lm(last5~tim[1:60]))   }
                        slope <- coef(res)[2,1]   # analyze trend in last 5 years
                        slopelim <- (1.0/ (length(last5))) * sd(co, na.rm=T)
                        if (slope >  slopelim)  {
                            arrows(0.98, 0.89, x1 = 1.02, y1 = 1.01, length = 0.08, angle = 45, code = 2, lwd = 3)
                        }
                        if (slope <  -slopelim) {
                            arrows(0.98, 1.01, x1 = 1.02, y1 = 0.89, length = 0.08, angle = 45, code = 2, lwd = 3)
                        }
                        if (slope <= slopelim & slope >= -slopelim) {
                            arrows(0.97, 0.95, x1 = 1.03, y1 = 0.95, length = 0.08, angle = 45, code = 3, lwd = 3)
                        }
                    }
                }
            ...
            if (length(tim) <= 5) {
                ...
                if (sameYscale==F)  {
                    ...
                    polygon(
                        c(min(tim_all)-5, max(tim_all)+5, max(tim_all)+5, min(tim_all)-5),
                    ...
                    if (mean(diff(tim_all)) <= 1 )  {  # plot time series
                        lines(tim_all, co_all, lwd=2);
                        points(tim_all, co_all, pch=20)
                    }
